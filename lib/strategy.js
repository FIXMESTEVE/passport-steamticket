...
SteamticketStrategy.prototype.authenticate = function(req, options) {
    ...
    request({
        ...
        if(error){
            return self.fail('request failed: ' + error);
        }
        else if (body.response.error) {
            return self.fail('auth failed: ' + body.response.error.errordesc);
        } else {
            ...
            try {
                if (self._passReqToCallback) {
                    self._verify(req, body.response.params, verified);
                } else {
                    self._verify(body.response.params, verified);
                }
            } catch(ex) {
                self.error(ex);
            }
